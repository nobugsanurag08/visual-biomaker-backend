...
export interface Template21HtmlOptions {
  baseURL?: string;
  assetBase?: string;
}

function getTemplate21Css(assetBase: string): string {
  const u = (file: string) =>
    assetBase ? `url('${assetBase}/${file}')` : `url('${file}')`;
  return `
body { margin: 0; padding: 0; }
.template21-container * { margin: 0; padding: 0; box-sizing: border-box; }
.template21-container h1 { font-size: 2.5rem; font-family: "Great Vibes", cursive; font-weight: 700; }
.template21-container p { font-size: 0.75rem; font-family: "Poppins", sans-serif; }
...
.template21-container .corner-svg {
  position: absolute; width: 280px; height: 280px; opacity: 0.9; pointer-events: none;
  background-image: ${u('goldenPhool.png')}; background-size: contain; background-repeat: no-repeat; background-position: center;
}
.template21-container .corner-top-left { top: -140px; left: -140px; transform: rotate(0deg); }
.template21-container .corner-top-right { top: -140px; right: -140px; transform: rotate(90deg); }
.template21-container .corner-bottom-left { bottom: -140px; left: -140px; transform: rotate(270deg); }
.template21-container .corner-bottom-right { bottom: -140px; right: -140px; transform: rotate(180deg); }
.template21-container .center-svg {
  position: absolute; width: 300px; height: 300px; opacity: 0.2; pointer-events: none;
  background-image: ${u('goldenPhool.png')}; background-size: contain; background-repeat: no-repeat; background-position: center;
}
.template21-container .center-top { top: -150px; left: 50%; transform: translateX(-50%) rotate(0deg); }
.template21-container .center-bottom { bottom: -150px; left: 50%; transform: translateX(-50%) rotate(180deg); }
.template21-container .center-decoration {
  position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 100%; max-width: 400px;
  height: 30%; opacity: 0.2; z-index: 1; pointer-events: none;
  background-image: ${u('ganesh.png')}; background-size: contain; background-repeat: no-repeat; background-position: center;
}
.template21-container .header {
  font-weight: bolder; display: flex; flex-direction: column; justify-content: center; align-items: center; margin-top: 1rem;
}
.template21-container .profile-image {
...
.template21-container .profile-image img { width: 100%; height: 100%; object-fit: cover; }
.template21-container .content {
  padding: 0 1rem; padding-top: 1rem; display: flex; flex-direction: column; align-items: center; text-align: center; z-index: 10;
}
.template21-container .row { display: flex; flex-direction: row; gap: 1rem; justify-content: center; }
.template21-container .info-right { width: 70%; height: fit-content; position: relative; padding-bottom: 1rem; top: 0; }
.template21-container .personal { padding-top: 1rem; }
.template21-container .family { padding-top: 1rem; }
.template21-container .details {
  margin-top: 1rem; text-align: left; display: grid; grid-template-columns: auto 1fr; row-gap: 10px; line-height: 1.6;
}
.template21-container .details div { display: contents; }
.template21-container .details span:first-child { padding-right: 2rem; font-weight: bold; }
.template21-container .details span:last-child { padding-right: 2rem; }
.template21-container .family-label { font-size: 13px; font-weight: 500; font-family: "Poppins", sans-serif; }
.template21-container .section-header {
...
.template21-container .section-header h3 { color: #33002a; margin: 0; font-weight: 600; letter-spacing: 3px; }
.template21-container .footer { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0; margin: 1rem 0 0.5rem 0; }
.template21-container .contact-text { display: flex; flex-direction: column; align-items: center; gap: 0.3rem; }
.template21-container .contact-icon { width: 15px; height: 15px; margin-right: 0.8rem; font-size: 13px; display: flex; align-items: center; }
.template21-container .contact-content { font-family: "Montserrat", sans-serif; font-size: 13px; }
.template21-container .contact-row { display: flex; flex-direction: row; align-items: center; }
`;
}

export function buildTemplate21Html(
  profileData: ProfileData,
  options?: Template21HtmlOptions
): string {
  const assetBase = options?.assetBase ?? '';
  const css = getTemplate21Css(assetBase);
...
<body>
  <div class="template21-container" id="biodata-container">
